...
# input
inPath = '/project/s1174/lyan/code/scratch/run/nucleon_sigma_term/cA211.53.24/QuarkLoops_pi0_insertion/data_out/'
outPath = './data_post/'
outPathMerged= './data_post_merged/'
...
for pat in patterns:
    ...
    with h5py.File(outPathMerged+pat+'.h5','w') as f:
        if pat == 'pi0Loop':
            ...
            for cfg in data.keys():
                f.create_dataset(cfg+'/mvec',data=momList)
                f.create_dataset(cfg+'/sigmaLoop',data=data[cfg][0,:,:,:]*1/np.sqrt(2))
