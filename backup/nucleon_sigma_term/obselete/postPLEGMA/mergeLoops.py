'''
nohup python3 -u mergeLoops.py > log/mergeLoops.out &
...
# input
inPath = '/p/project/pines/li47/code/scratch/run/nucleon_sigma_term/cA211.53.24/QuarkLoops_pi0_insertion/output_data/'
outPath = './avg/'
...
for pat in patterns:
    ...
    for cfg in cfgList:
        ...
        with h5py.File(getFilePath(cfg,pat)) as f:
            ...
            for stoc in f[src].keys():
                ...
                Nstoc+=t_stoc
                t=f[src][stoc][fla][:,:,:,0]+1j*f[src][stoc][fla][:,:,:,1]
    ...
    with h5py.File(outPath+pat+'.h5','w') as f:
        if pat == 'pi0Loop':
            for cfg in data.keys():
